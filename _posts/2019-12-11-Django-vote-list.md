...
로그인 했을 떄 헤더에는 어떠한 값, 바디에는 어떠한 값, 또한 어떤 상황애서는 어떠한 값을 호출해야 하는지 패턴을.. 정해야 하는 번거로움이 있지만...! Rest api는 이러한 문제를 해결할 수 있다.


POSTMAN 이라는 툴을 사용하기 이전에는 문서를 사용해야 했었는데, 이는 시간과 에너지가 많이 드는 일이다...
