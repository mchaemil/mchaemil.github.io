...
네이버 주소를 요청하면 네이버에서 웹사이트에 대한 응답을 주면서 헤더(header)라는 곳에 쿠키(cookie)라는 정보를 넣어서 준다. 클라이언트는 쿠키정보를 내 쿠키 저장소에 저장을 한다.
...
Client에서는 로그인한 사용자 정보를 가지고 있어야 한다. 로그인을 한 후에는 내가 로그인한 사용자라는 것을 서버가 인지해야 한다. 그리고 원하는 정보를 디비에서 꺼내서 쓸 수 있어야 한다. 이를 위해서 쿠키가 사용된다.

- 쿠키는 Client에서 유지해야 하는 데이터를 유지할 수 있도록 돕는다. 
- 쿠키에 사용되는 키를 가지고 Client를 구분할 수 있다.


#### Client 요청에 의해 쿠키가 사용되는 흐름

Client의 요청에 의해 Server는 Client가 쿠키로 사용할 키를 생성해 문자열을 만들고 나서 데이터베이스에 등록을 한 후, Client에게 알려준다. Client는 할당받은 문자열을 자기 저장소에 저장을 한다(해당 사이트에서만 사용할 키) 

그 다음 요청부터는 Client는 모든 요청에 쿠키에 저장한 키를 같이 보낸다. 이를 통해서 Server는 쿠키에 저장된 키를 통해 사용자를 기억할 수 있게 된다. 그리고 Client는 원하는 정보를 DB에서 꺼내서 쓰게 된다.

이를 통해 Server에서는 Client를 구별해서 인지할 수 있고, Server는 Client가 가지고 있는 키를 통해서 Client를 구분할 수 있다. 
