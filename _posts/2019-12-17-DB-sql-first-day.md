...
SQL의 데이터를 정제할 수 있는 다양한 문법을 학습하고 이를 활용하여 데이터를 정제하는 방법을 연습해보았다.
주어진 SQL 문제를 풀어보면서, SQL을 익히고, 익숙해지는 데는 충분한 시간이 필요하며, '내가 풀어볼 수 있을 것 같은데?'와 같은 감각을 잃지 않는 것도 중요하다는 생각이 들었다.

결국, 내가 목표해야 하는 지점은 테이블에서 올바르게 조건을 지정해서 원하는 데이터를 뽑아낼 수 있는 능력이다. 한 번에 완벽한 쿼리를 작성하는 것은 어려운 작업이다. 어떻게 쿼리를 구성할 지 고민하기 전에 데이터가 있는지, 없는지를 확인하며, 차근차근 커리를 만들어가는 연습을 진행해야겠다. 천리길도 한 걸음부터 시작하는 법이므로
